...
function GetCurrentParam(playerID)
	local era = 0
	if Game.GetEras then
		era = Game.GetEras():GetCurrentEra()
	else
		era = Players[playerID]:GetEras():GetEra()
	end
...
function AddScienceCulture(playerID, iX, iY)
	local pPlayer = Players[playerID]
	local params = GetCurrentParam(playerID)
