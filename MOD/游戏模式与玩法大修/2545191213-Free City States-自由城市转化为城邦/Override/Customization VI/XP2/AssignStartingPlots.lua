...
local iPercentageDeletedCityStates = 50 -- a higher number leads to less city states starting in the beginning of the game; note that this number should be balanced with the number of city states in MapSizes.sql
...
function DeleteUnitsOnMap ( iPlayerID )
	UnitManager.InitUnit(iPlayerID  , "UNIT_SETTLER", -1, -1);	-- this should happen even before city state unit spawn on map. because of this offmap spawn it seems that the city state units do not spawn on the map in the beginning of the game so they do not have a starting position that gets revealed when a player becomes suzerain of this city state (this was a problem in FreeCityStates.lua)
end
...
local kDeletedCityStates = {}
function IsDeletedCityState( iPlayerID )
	for _, iDeletedCityStateID in ipairs( kDeletedCityStates ) do
		if( iPlayerID == iDeletedCityStateID ) then
			return true;
		end
	end	
	return false;
...
function GetPlayerIDFromStartingPlot( pPlot )
	for iPlayerID = 0, GameDefines.MAX_PLAYERS-1, 1 do
		if( pPlot == Players[iPlayerID]:GetStartingPlot() ) then
			return iPlayerID;
		end
	end
	return -1; --no player
...
function MarkPlotsNearStartLocations( kInstance )
	local iXMax, iYMax = Map.GetGridSize()
	iXMax, iYMax = iXMax - 1, iYMax - 1 --subtract 1 because coordinates start at 0, 0 instead of 1, 1
	local OverlapCounts = {}
	local CityRange = GlobalParameters.CITY_MIN_RANGE
	local iMinorMinDistance = math.floor( 1 * CityRange )
	local iMajorMinDistance = math.floor( 2 * CityRange )
	for iX = 0,iXMax do
		OverlapCounts[iX] = {}
		for iY = 0,iYMax do
			OverlapCounts[iX][iY] = 0
			--check minor civ start positions:
			for _, pStartPlot in ipairs(kInstance.minorStartPlots) do
				local iDistance = Map.GetPlotDistance(iX, iY, pStartPlot:GetX(), pStartPlot:GetY())
				if( iDistance <= iMinorMinDistance ) then
					OverlapCounts[iX][iY] = OverlapCounts[iX][iY] + 1
				end				
			end
			--check major civ start positions:
			for _, pStartPlot in ipairs(kInstance.majorStartPlots) do
				local iDistance = Map.GetPlotDistance(iX, iY, pStartPlot:GetX(), pStartPlot:GetY())
				if( iDistance <= iMajorMinDistance ) then
					OverlapCounts[iX][iY] = OverlapCounts[iX][iY] + 1
				end	
			end		
...
function DeleteMostClusteredCityState( bDeleteEnabled, kInstance )
-- Deletes City State that has the least distance to other city state settlers (requires that city states did not found a city yet)
	...
	local OverlapCounts = MarkPlotsNearStartLocations( kInstance )
	for _, pPlot in ipairs(kInstance.minorStartPlots) do
		local iCityStateID = GetPlayerIDFromStartingPlot( pPlot )
		if( iCityStateID ~= -1 and not IsDeletedCityState(iCityStateID) and not Players[iCityStateID]:IsMajor() ) then			
			if ( not HasMetMajorCivilization(iCityStateID) ) then
				local DistanceSum = 0;
				local iOverlapCount = OverlapCounts[pPlot:GetX()][pPlot:GetY()]
				if( iOverlapCount == 0 ) then
					for _, pNeighbouredCityStatePlot in ipairs(kInstance.minorStartPlots) do		
						local iNeighbouredCityStateID = GetPlayerIDFromStartingPlot( pPlot )
						if ( iNeighbouredCityStateID ~= -1 and not IsDeletedCityState(iNeighbouredCityStateID) ) then
							local Distance = Map.GetPlotDistance(pPlot:GetX(), pPlot:GetY(), pNeighbouredCityStatePlot:GetX(), pNeighbouredCityStatePlot:GetY())			
							DistanceSum = DistanceSum + Distance	
						end
					end
				else
					DistanceSum = DistanceSum - iOverlapCount
				end
	...
	if ( bDeleteEnabled ) then
		...
		if ( iPlayerDelete ) then
			...
			table.insert(kDeletedCityStates, iPlayerDelete)
			print("Deleting units of player "..iPlayerDelete .. " with SumDistance " .. CityStateDistanceSums[iPlayerDelete])			
		end
	end
...
function TiramodCreateCityStatePool( kInstance )
	local iNumStartingMinorCivs = math.floor( PlayerManager.GetAliveMinorsCount() * iPercentageDeletedCityStates / 100 );		
	
	for iPlayerID = kInstance.iNumMajorCivs - 1 + iNumStartingMinorCivs, kInstance.iNumMajorCivs - 2 + PlayerManager.GetAliveMinorsCount() do		
		DeleteMostClusteredCityState(true, kInstance)	
	end
end
...
function Initialize() 
	print("Start Tiras AssignStartingPlotAlgorithmLuaScript Customization VI XP2")
end
Initialize();
...
--			print ("Minor SouthEdge: " .. tostring(info.SouthEdge));
	...
	TiramodCreateCityStatePool( self ) -- #Tiramod Change
...
function AssignStartingPlots:__MajorCivBuffer(plot)
	...
	-- Zegangani, Custom Distance between Major Civs:
	--local iMaxStart = GlobalParameters.START_DISTANCE_MAJOR_CIVILIZATION or 9;
	local iMaxStart = MapConfiguration.GetValue("StartDistanceMajorCivs") or 12;
...
function AssignStartingPlots:__MinorMajorCivBuffer(plot)
	...
	-- Zegangani, Custom Distance between Major and Minor Civs:
	--local iMaxStart = GlobalParameters.START_DISTANCE_MINOR_MAJOR_CIVILIZATION or 6;
	local iMaxStart = MapConfiguration.GetValue("StartDistanceMinorMajorCivs") or 6;
...
function AssignStartingPlots:__MinorMinorCivBuffer(plot)
	...
	-- Zegangani, Custom Distance between Minor Civs:
	--local iMaxStart = GlobalParameters.START_DISTANCE_MINOR_CIVILIZATION_START or 5;
	local iMaxStart = MapConfiguration.GetValue("StartDistanceMinorCivs") or 5;
