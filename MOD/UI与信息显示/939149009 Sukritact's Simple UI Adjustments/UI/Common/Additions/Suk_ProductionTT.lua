...
--------------------------------------------------------------
local iQueueLength_Cached;
function UpdateSuk_CityBannerProductionTT(tControl, pCity)

	local pBuildQueue = pCity:GetBuildQueue()

	local iQueueLength = 0
	while pBuildQueue:GetAt(iQueueLength) do
		iQueueLength = iQueueLength + 1;
	end
	if iQueueLength ~= iQueueLength_Cached then
		iQueueLength_Cached = iQueueLength
	...
	for i = 0, 5 do
		...
		if i == 0 then
			...
			local tInstance = m_Suk_QueueIM:GetAllocatedInstance(i)
			if not tInstance then tInstance = m_Suk_QueueIM:GetInstance() end
