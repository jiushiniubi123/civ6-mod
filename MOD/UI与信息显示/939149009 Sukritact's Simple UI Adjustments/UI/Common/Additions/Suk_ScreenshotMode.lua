...
function EnterScreenshotMode()
	...
	-- ResourceIcons
	if ExposedMembers.Suk_ScreenshotMode_Options.HideResourceIcons then
		pControl = ContextPtr:LookUpControl("/InGame/WorldViewIconsManager")
		ExposedMembers.Suk_ScreenshotMode_StoredStates[pControl] = pControl:IsHidden()
		pControl:SetHide(true)
	end

	-- CityBanners
	if ExposedMembers.Suk_ScreenshotMode_Options.HideMapPins then
		pControl = ContextPtr:LookUpControl("/InGame/MapPinManager")
		ExposedMembers.Suk_ScreenshotMode_StoredStates[pControl] = pControl:IsHidden()
		pControl:SetHide(true)
	end
	----------------------------
...
function ExitScreenshotMode()
	...
	UI.SetAmbientTimeOfDay(m_CachedTimeOfDay)
...
function OnInit( bIsReload)
	...
	for i,pChild in pairs(pWorldViewControls:GetChildren()) do
		if not m_ImmuneContexts[pChild:GetID()] then
