...
local EDR_showTradeDealsIndicator = true; --ARISTOS: to toggle Deal Indicator ON/OFF
...
function OnLeaderMouseOver(playerID : number )
  ...
  if m_isCTRLDown then
    ...
    for otherPlayerID, instance in pairs(m_uiLeadersByID) do
	  ...
	  instance.DealIndicator:SetHide(true);
    end
  end
end
...
function GetExtendedTooltip(playerID:number)
  ...
  --Player Religion
  local playerReligionType = Players[playerID]:GetReligion():GetReligionInMajorityOfCities();
  local isFounder:boolean = false;
  for _, tmp in ipairs(Game.GetReligion():GetReligions()) do
		if (tmp.Religion == playerReligionType and tmp.Founder == playerID) then
			isFounder = true;
			break;
		end
  end
  ...
  local civData:string = sEras
	...
	.."[NEWLINE]"..Locale.Lookup("LOC_HUD_CITY_RELIGION") .. ": " .. 
		(playerReligionType ~= -1 and  Locale.ToUpper(Game.GetReligion():GetName(playerReligionType))..(isFounder and " [ICON_Religion]" or "") or Locale.Lookup("LOC_TECH_FILTER_NONE"))
  ...
  -- ARISTOS: moved to its own DealIndicator icon!
  local deals: string = AddTradeDeals(playerID);
  if deals ~= nil then
	civData = civData .. "[NEWLINE]-----------------------------------------------[NEWLINE]" .. Locale.Lookup("LOC_GOSSIPDESC_TRADE_DEAL") .. ":" .. deals;
  end
		...
		for i,pDeal in ipairs(pDeals) do
			...
			--if pDeal:IsValid() then  --ARISTOS: BUGGED as Hell!!!! Shame on you Firaxis...
				...
				if remainingTurns ~= nil then
					tradeDealsTooltip = tradeDealsTooltip == nil and "[NEWLINE]" or tradeDealsTooltip .. "[NEWLINE]"
...
function AddLeader(iconName : string, playerID : number, kProps: table)
	...
	local dealsTooltip: string = EDR_showTradeDealsIndicator and AddTradeDeals(playerID) or nil;
	...
	instance.DealIndicator:SetToolTipString(dealsTooltip == nil and "" or Locale.Lookup("LOC_GOSSIPDESC_TRADE_DEAL") .. ":" .. dealsTooltip);
	...
	if(pPlayerConfig ~= nil) then
		...
		if(leaderTypeName ~= nil) then
			...
			local isMasked	:boolean = (isMet==false) and isHumanMP;	-- Multiplayer human but haven't met yet
			-- Append GetExtendedTooltip string to the end of the tooltip created by LeaderIcon if it is not masked
