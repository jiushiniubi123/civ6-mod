...
--Better Unit List
--Black Death Mod
--AB2531 Plague Mod BACKUP
--Enhanced Heal Tooltip
--AB2531 Plague Mod Hotfix
--Enhanced Heal Tooltip

--xml层面
--DB's Civ 6 Multiplayer Gold
-- ===========================================================================
--	Unit Panel Screen 
-- ===========================================================================
...
local m_ShouldShowHigherModifierStack_BCI = false
local m_bci_currentLanguage	= Locale.GetCurrentLanguage().Type;
-- ===========================================================================
--	FUNCTIONS
-- ===========================================================================
...
-- Third Party
-- EnhancedHealTooltip
local m_EnhancedHealTooltipInUse = Modding.IsModActive("ac8447ac-7ed8-43f0-8e8a-a8295deaa3a6");
-- ===========================================================================
-- EnhancedHealTooltip: return the heal tooltip based on the given damage
function GetHealTooltip( damage :number, unitDomain :string, isEmbarked :boolean, plot :table )
	if (damage == 0) then
		return "";
	end

	local cty :string = "CITY_KEY";
	local fri :string = "FIRENDLY_KEY";
	local neu :string = "NEUTRAL_KEY";
	local riv :string = "RIVAL_KEY";

	-- init tooltip parts
	listItemPrefix = {}
	listItemPrefix[cty] = "[ICON_BULLET]";
	listItemPrefix[fri] = "[ICON_BULLET]";
	listItemPrefix[neu] = "[ICON_BULLET]";
	listItemPrefix[riv] = "[ICON_BULLET]";

	color = {};
	color[cty] = "[COLOR_MEDIUM_GREEN]";
	color[fri] = "[COLOR_ModStatusGreen]";
	color[neu] = "[COLOR:GrayMedium]";
	color[riv] = "[COLOR:Red]";
	...
	turns = {};
	turns[cty] = TurnsToHeal(damage, 20);
	turns[fri] = TurnsToHeal(damage, 15);
	turns[neu] = TurnsToHeal(damage, 10);
	turns[riv] = TurnsToHeal(damage, 5);
	...
	elseif (unitDomain == "DOMAIN_SEA") then
		turns[fri] = TurnsToHeal(damage, 20);
		turns[neu] = nil;
		turns[riv] = nil;
	...
	-- Find out in which territory the unit currently is
	if (plot:IsCity()) then
		-- can only enter our own cities
		listItemPrefix[cty] = "[Icon_GoingTo]";
	else
		local ownerId :number = plot:GetOwner();
		if (ownerId == -1) then
			listItemPrefix[neu] = "[Icon_GoingTo]";
		else
			local owner :table = Players[ownerId];
			local localPlayerId :number = Game.GetLocalPlayer();
			local isLocalPlayer :boolean = ownerId == localPlayerId;
			local isMajor :boolean = owner:IsMajor();
			local isAllied :boolean = isMajor and GameInfo.DiplomaticStates[owner:GetDiplomaticAI():GetDiplomaticStateIndex( 0 )].Hash == DiplomaticStates.ALLIED;
			local isSuzerain :boolean = not isMajor and owner:GetInfluence():GetSuzerain() == localPlayerId;
			if (isLocalPlayer or isAllied or isSuzerain) then
				listItemPrefix[fri] = "[Icon_GoingTo]";
			else
				listItemPrefix[riv] = "[Icon_GoingTo]";
			end
		end
	end

	-- generate the tooltip
	local tooltip :string = listItemPrefix[cty] .. color[cty] .. "City:          " .. turnPrefix[cty] .. turns[cty] .. turnPostfix[cty] .. "[ENDCOLOR] [ICON_TURN]";
	tooltip = tooltip .. "[NEWLINE]" .. listItemPrefix[fri] .. color[fri] .. "Friendly: " .. turnPrefix[fri] .. turns[fri] .. turnPostfix[fri] .. "[ENDCOLOR] [ICON_TURN]";
	if (turns[neu] ~= nil) then
		tooltip = tooltip .. "[NEWLINE]" .. listItemPrefix[neu] .. color[neu] .. "Neutral:   " .. turnPrefix[neu] .. turns[neu] .. turnPostfix[neu] .. "[ENDCOLOR] [ICON_TURN]";
	end
	if (turns[riv] ~= nil) then
		tooltip = tooltip .. "[NEWLINE]" .. listItemPrefix[riv] .. color[riv] .. "Rival:        " .. turnPrefix[riv] .. turns[riv] .. turnPostfix[riv] .. "[ENDCOLOR] [ICON_TURN]";
	end

	return tooltip;
end

-- EnhancedHealTooltip: helper
function TurnsToHeal( damage :number, healRate :number )
	return math.floor(damage / healRate) + BoolToNumber(damage % healRate > 0);
end
			...
			elseif (actionHash == UnitOperationTypes.WMD_STRIKE) then
				...
				if ( operationRow.VisibleInUI ) then
					...
					if (bCanStart) then
						...
						--兼容EnhancedHealTooltip
						if m_EnhancedHealTooltipInUse then
							local isHeal :boolean = operationRow.Description == "LOC_UNITOPERATION_HEAL_DESCRIPTION" or operationRow.Description == "LOC_UNITOPERATION_REST_REPAIR_DESCRIPTION";
							local isReligious :boolean = pUnit:GetReligiousStrength() > 0;
							local requiredResource :string = GameInfo.Units[unitType].StrategicResource;
							local isResourceMissing :boolean = false;

							if (isHeal and not isReligious) then
								-- check if player has the required strategic resource
								if (requiredResource ~= nil and Players[Game.GetLocalPlayer()]:GetResources():GetResourceAmount(requiredResource) == 0) then
									isResourceMissing = true;
									bCanStart = false;
								else
									local plot :table = Map.GetPlot(pUnit:GetX(), pUnit:GetY());
									toolTipString = toolTipString .. "[NEWLINE]"
										.. GetHealTooltip(pUnit:GetDamage(), GameInfo.Units[unitType].Domain, pUnit:IsEmbarked(), plot);
								end
							end
						end
...
function RealizeHealthMeter( control:table, percent:number, controlShadow:table, shadowPercent:number )
	...
	--print("shadowPercent=\t", shadowPercent)
...
--Maple_Leaves Better Combat Info
function RealizeHealthMeter_BetterCombatInfo( control:table, percent:number, controlShadow2:table, shadowPercent2:number , controlShadow:table, shadowPercent:number )
	
	--实际上是做三重差分
	if	( percent > 0.7 )	then 
		control:SetColor( COLORS.METER_HP_GOOD );	
		controlShadow2:SetColor( METER_HP_GOOD_MID_SHADOW );	
		controlShadow:SetColor( COLORS.METER_HP_GOOD_SHADOW );	
	elseif ( percent > 0.4 )	then
		control:SetColor( COLORS.METER_HP_OK );
		controlShadow2:SetColor( METER_HP_OK_MID_SHADOW );	
		controlShadow:SetColor( COLORS.METER_HP_OK_SHADOW );	
	else						 
		control:SetColor( COLORS.METER_HP_BAD ); 
		controlShadow2:SetColor( METER_HP_BAD_MID_SHADOW );	
		controlShadow:SetColor( COLORS.METER_HP_BAD_SHADOW );	
	end
	...
	control:SetPercent( percent );
	controlShadow2:SetPercent( shadowPercent2 );
	controlShadow:SetPercent( shadowPercent );
end
-- ===========================================================================
...
function ViewTarget(data)
	...
	if (data.MaxWallDamage > 0) then
		...
		--Maple_Leaves Better Combat Info
		local showCombatDetails, m_combatType = CanShowCombatDetail_BetterCombatInfo()
		if showCombatDetails then
			local min_combat_damage, max_combat_damage = GetPossibleCombatDamage_BetterCombatInfo(false)

			local percent			:number = 1 - GetPercentFromDamage( data.Damage + max_combat_damage,	data.MaxDamage );
			local shadowPercent2	:number = 1 - GetPercentFromDamage( data.Damage + min_combat_damage,	data.MaxDamage );
			local shadowPercent		:number = 1 - GetPercentFromDamage( data.Damage,						data.MaxDamage );

			RealizeHealthMeter_BetterCombatInfo( Controls.TargetHealthMeter, percent, 
				Controls.TargetHealthMeterShadow2, shadowPercent2, 
				Controls.TargetHealthMeterShadow, shadowPercent 
			)

			Controls.TargetHealthMeterShadow2:SetHide(false);
			...
			Controls.TargetHealthMeterShadow2:SetHide(true);
		end
...
function OnShowCombat( showCombat )
	...
	if (showCombat) then
		...
		local showCombatDetails, m_combatType = CanShowCombatDetail_BetterCombatInfo()
		if showCombatDetails and m_combatType == CombatTypes.MELEE then
			--注意，必须是近战才会受到反伤
			--应该不是很严谨，因为空袭遇到防空会变得很奇怪，但是不考虑它也不算错就是了，最多算不完善
			local min_combat_damage, max_combat_damage = GetPossibleCombatDamage_BetterCombatInfo(true)

			local percent			:number = 1 - GetPercentFromDamage(		attackerCurrentDamage + max_combat_damage,	attackerMaxDamage );
			local shadowPercent2	:number = 1 - GetPercentFromDamage(		attackerCurrentDamage + min_combat_damage,	attackerMaxDamage );
			local shadowPercent		:number = 1 - GetPercentFromDamage(		attackerCurrentDamage,						attackerMaxDamage );

			RealizeHealthMeter_BetterCombatInfo( Controls.UnitHealthMeter, percent, 
				Controls.UnitHealthMeterShadow2, shadowPercent2, 
				Controls.UnitHealthMeterShadow, shadowPercent
			)

			Controls.UnitHealthMeterShadow2:SetHide(false);
			...
			Controls.UnitHealthMeterShadow2:SetHide(true);

		end

		--战斗力buff列表
		if showCombatDetails then
			local  att_modifierList, att_modifierListSize = GetCombatModifierList(CombatResultParameters.ATTACKER );
			local  def_modifierList, def_modifierListSize = GetCombatModifierList(CombatResultParameters.DEFENDER );
			BuildCombatStrengthModifiers_BCI(
				Controls.UnitCombatStrength_ModifierList_BG_BCI,  Controls.UnitCombatStrength_ModifierList_Text_BCI, 
				false, att_modifierList, att_modifierListSize
			)
			BuildCombatStrengthModifiers_BCI(
				Controls.TargetCombatStrength_ModifierList_BG_BCI,  Controls.TargetCombatStrength_ModifierList_Text_BCI, 
				false, def_modifierList, def_modifierListSize
			)
		else
			Controls.UnitCombatStrength_ModifierList_BG_BCI:SetHide(true)
			Controls.TargetCombatStrength_ModifierList_BG_BCI:SetHide(true)
		end
...
function CanShowCombatDetail_BetterCombatInfo()
	if m_combatResults == nil then
		return false, CombatTypes.MELEE
	end

	--只有在单位打单位时候才生效，且必须是近战/远程/轰炸
	--进攻者必须不是空军(不然会很复杂)
	--很乐的是，战斗机是远程攻击，轰炸机是轰炸攻击
	...
	if (defenderID.type == ComponentType.UNIT) then
		...
		if (pkDefender ~= nil) and iDefenderStrength > 0 then
			...
			local pAttacker :table= UI.GetHeadSelectedUnit();
			if pAttacker == nil then
				return false, CombatTypes.MELEE
			end
			local attacker_typeStr = pAttacker:GetType()
			local attacker_domain = GameInfo.Units[attacker_typeStr].Domain
			
			if attacker_domain ~= "DOMAIN_AIR" then
				--进攻者必须不是空军
				if combatType == CombatTypes.MELEE or combatType == CombatTypes.RANGED or combatType == CombatTypes.BOMBARD then
					--必须是近战/远程/轰炸
	...
	return showCombatDetails, combatType
end

function GetDefenderUnitName_BetterCombatInfo()
	local defender = m_combatResults[CombatResultParameters.DEFENDER];		
	local defenderID = defender[CombatResultParameters.ID];
	local pkDefender = UnitManager.GetUnit(defenderID.player, defenderID.id);
	local defender_typeStr = pkDefender:GetType()
	local defender_name = GameInfo.Units[defender_typeStr].Name

	local defender_damage		= pkDefender:GetDamage();
	local defender_max_damage	= pkDefender:GetMaxDamage();

	return defender_name, defender_damage, defender_max_damage
end

function GetAttackerUnitName_BetterCombatInfo()
	local pAttacker :table= UI.GetHeadSelectedUnit();
	local attacker_typeStr = pAttacker:GetType()
	local attacker_name = GameInfo.Units[attacker_typeStr].Name

	local attacker_damage		= pAttacker:GetDamage();
	local attacker_max_damage	= pAttacker:GetMaxDamage();

	return attacker_name, attacker_damage, attacker_max_damage
end
...
function GetPossibleCombatDamage_BetterCombatInfo(reverse)
	...
	if reverse then
		combat_Difference = - combat_Difference
	end
	...
	local max_combat_damage = math.floor(
		(base_combat_damage+max_extra_combat_damage)*math.exp(combat_power_scaling*combat_Difference)
	)
...
function ShowCombatAssessment()
	...
	local tooltip_bci_text = "";
	local showCombatDetails, m_combatType = CanShowCombatDetail_BetterCombatInfo()
	if showCombatDetails then
		--获取单位名字和生命
		local defender_name, defender_damage, defender_max_damage = GetDefenderUnitName_BetterCombatInfo()
		local attacker_name, attacker_damage, attacker_max_damage = GetAttackerUnitName_BetterCombatInfo()

		local defender_health = defender_max_damage - defender_damage
		local attacker_health = attacker_max_damage - attacker_damage

		local min_combat_damage, max_combat_damage = GetPossibleCombatDamage_BetterCombatInfo(false)
		local resultText = Locale.Lookup("LOC_HUD_UNIT_PANEL_OUTCOME_POSSIBLE_RESULT", Locale.Lookup(attacker_name), min_combat_damage, max_combat_damage);
		tooltip_bci_text = tooltip_bci_text .. resultText
		tooltip_bci_text = tooltip_bci_text .. "[NEWLINE]"
		if m_bci_currentLanguage == "zh_Hans_CN" then
			tooltip_bci_text = tooltip_bci_text .. "[NEWLINE]"
		end

		local min_combat_damage_r = 0
		local max_combat_damage_r = 0
		if combatType == CombatTypes.MELEE then	
			min_combat_damage_r, max_combat_damage_r = GetPossibleCombatDamage_BetterCombatInfo(true)
			local resultReverseText = Locale.Lookup("LOC_HUD_UNIT_PANEL_OUTCOME_POSSIBLE_RESULT_REVERSE", Locale.Lookup(attacker_name), min_combat_damage_r, max_combat_damage_r);
			tooltip_bci_text = tooltip_bci_text .. resultReverseText
			tooltip_bci_text = tooltip_bci_text .. "[NEWLINE]"
			if m_bci_currentLanguage == "zh_Hans_CN" then
				tooltip_bci_text = tooltip_bci_text .. "[NEWLINE]"
			end
		...
		local defender_result = ""
		if defender_health - min_combat_damage <= 0 then
			defender_result = Locale.Lookup("LOC_HUD_UNIT_PANEL_OUTCOME_JUDGEMENT_DEFENDER_DIE", Locale.Lookup(defender_name));
		elseif (defender_health - min_combat_damage > 0) and (defender_health - max_combat_damage <= 0) then
			defender_result = Locale.Lookup("LOC_HUD_UNIT_PANEL_OUTCOME_JUDGEMENT_DEFENDER_POSSIBLE_DIE", Locale.Lookup(defender_name));
		else
			defender_result = Locale.Lookup("LOC_HUD_UNIT_PANEL_OUTCOME_JUDGEMENT_DEFENDER_LIVE", Locale.Lookup(defender_name));
		end
		tooltip_bci_text = tooltip_bci_text .. defender_result
		tooltip_bci_text = tooltip_bci_text .. "[NEWLINE]"
		if m_bci_currentLanguage == "zh_Hans_CN" then
			tooltip_bci_text = tooltip_bci_text .. "[NEWLINE]"
		end
		...
		if combatType == CombatTypes.MELEE then
			local attacker_result = ""
			if attacker_health - min_combat_damage_r <= 0 then
				attacker_result = Locale.Lookup("LOC_HUD_UNIT_PANEL_OUTCOME_JUDGEMENT_ATTACKER_DIE", Locale.Lookup(attacker_name));
			elseif (attacker_health - min_combat_damage_r > 0) and (attacker_health - max_combat_damage_r <= 0) then
				attacker_result = Locale.Lookup("LOC_HUD_UNIT_PANEL_OUTCOME_JUDGEMENT_ATTACKER_POSSIBLE_DIE", Locale.Lookup(attacker_name));
			else
				attacker_result = Locale.Lookup("LOC_HUD_UNIT_PANEL_OUTCOME_JUDGEMENT_ATTACKER_LIVE", Locale.Lookup(attacker_name));
			end
			tooltip_bci_text = tooltip_bci_text .. attacker_result
		end

		Controls.CombatPossibleResultText_BCI:SetText(tooltip_bci_text);	
		Controls.CombatPossibleResultBG_BCI:SetHide(false)

		--单位的生命值
		local unit_health_info = Locale.Lookup("LOC_HUD_UNIT_PANEL_OUTCOME_SELF_UNIT_HEALTH", attacker_health, attacker_max_damage)
		Controls.UnitHealth_Text_BCI:SetText(unit_health_info);	
		Controls.UnitHealth_BG_BCI:SetHide(false)

		local target_unit_health_info = Locale.Lookup("LOC_HUD_UNIT_PANEL_OUTCOME_TARGET_UNIT_HEALTH", defender_health, defender_max_damage)
		Controls.TargetHealth_Text_BCI:SetText(target_unit_health_info);	
		Controls.TargetHealth_BG_BCI:SetHide(false)

		--力差
		local combat_difference = iAttackerStrength - iDefenderStrength 
		local combat_difference_info = ""
		if combat_difference >= 0 then
			combat_difference_info = Locale.Lookup("LOC_HUD_UNIT_PANEL_OUTCOME_STRENGTH_DIFFERENCE_POSITIVE", combat_difference)
		else
			combat_difference_info = Locale.Lookup("LOC_HUD_UNIT_PANEL_OUTCOME_STRENGTH_DIFFERENCE_NEGATIVE", combat_difference)
		end
		...
		Controls.CombatPossibleResultBG_BCI:SetHide(true)

		Controls.UnitHealth_BG_BCI:SetHide(true)
		Controls.TargetHealth_BG_BCI:SetHide(true)
		Controls.CombatStrength_Difference_BG_BCI:SetHide(true)
...
-- ===========================================================================
function BuildCombatStrengthModifiers_BCI(bci_background:table,  bci_text:table, hide:boolean, modifierList:table, modifierCount:number)
	if hide then
		bci_background:SetHide(true);
		return
	end
	...
	for i=0, modifierCount, 1 do
		if modifierList[i] ~= nil then
			...
			if modifier_text ~= nil and modifier_icon ~= nil then
				valid_modifier_num = valid_modifier_num + 1
				if i > 0 then
					label_text = label_text .. "[NEWLINE]" 
				end

				local textureOffsetX_22, textureOffsetY_22, textureSheet_22 = IconManager:FindIconAtlas(modifier_icon, 22);
				local textureOffsetX_16, textureOffsetY_16, textureSheet_16 = IconManager:FindIconAtlas(modifier_icon, 16);
				--print("modifier_icon=", modifier_icon)
				--print("textureSheet_16=", textureSheet_16)
				--print("textureSheet_22=", textureSheet_22)

				if textureSheet_22 ~= nil then
					if textureSheet_16 ~= nil then
						label_text = label_text ..  "[ICON_" .. modifier_icon .."]" .. " ".. modifier_text
					else
						label_text = label_text ..  "[" .. modifier_icon .."]" .. " ".. modifier_text
					end
					--label_text = label_text ..  "[ICON_" .. modifier_icon .."]" .. " ".. modifier_text
				else
	...
	if label_text ~= "" and valid_modifier_num >= 5 then
...
function OnInputHandler( pInputStruct:table )
	...
	elseif uiMsg == MouseEvents.MButtonUp then 
		--Maple_Leaves Better Combat Info

		--放弃了快捷键，因为会和builder lag fix冲突
		--如果是ctrl键盘，则额外刷新一次
		local takeplace = 0
		--local key:number = pInputStruct:GetKey();
		--m_ShouldShowHigherModifierStack_BCI = not m_ShouldShowHigherModifierStack_BCI
		--print("switching to\t", m_ShouldShowHigherModifierStack_BCI)
		--InspectWhatsBelowTheCursor();
	end
