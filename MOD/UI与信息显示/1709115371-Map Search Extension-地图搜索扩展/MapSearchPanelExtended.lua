-- MapSearchPanelExtended
-- This file is the copy of base game file with modifications made by Zur13
-- DateCreated: 4/8/2019 2:46:48 AM
--------------------------------------------------------------
include( "InstanceManager" );

-- =================================================================================================================================================================================================================================
-- Vanilla code with Zur13 changes
-- =================================================================================================================================================================================================================================
...
local MAXIMUM_SEARCH_HISTORY = 8; -- Zur13 from 5 to 8
...
function IncrementalSearch()
	...
	for iPlot = m_nLastPlotSearched + 1, nPlots - 1 do
		...
		if pPlayerVis:IsRevealed(iPlot) and IsTileMatchesCriteria( iPlot ) then -- Zur13: added IsTileMatchesCriteria()
...
function UpdateSuggestions(pEditBox, pSuggestionPanel, pInstanceManager, pHistoryList)
	...
	if g_bSuggestionsInitialized then
		local results = Search.Search(SEARCHCONTEXT_SUGGESTIONS, str, 14); -- Zur13 from 10 to 14
...
function OnSearchCommit()
	...
	if ValidateSearchTerms() and g_bMapSearchInitialized then
		...
		if (pOverlay ~= nil) then
			...
			pOverlay:SetBorderColors(0, borderColor1, borderColor2); -- Zur13: changed COLOR_WHITE COLOR_WHITE
			pOverlay:SetHighlightColor(0, fillColor); -- Zur13: changed COLOR_GREEN
		end
	end
end
...
--Initialize(); -- Zur13: commented out this function because there is a copy of it bellow
...
--******************************************************************************
-- Zur13: is Expansion 2 Gathering Storm active
function IsXP2()
...
if IsXP2() then -- Zur13: override base functions only in case of XP2 ruleset
...
-- =================================================================================================================================================================================================================================
-- Zur13 Map Search Options
-- =================================================================================================================================================================================================================================

borderColor1 = COLOR_WHITE; 
borderColor2 = COLOR_WHITE; 
fillColor = COLOR_GREEN;

include( "MseSearchOptions" );

-- =================================================================================================================================================================================================================================
-- Zur13 Tile checkboxes
-- =================================================================================================================================================================================================================================


include( "MseTileCriteria" );

-- =================================================================================================================================================================================================================================
-- Zur13 Term destination
-- =================================================================================================================================================================================================================================

termDestinationEditBox = Controls.MapSearchBox; -- Controls.MapSearchBox Controls.MapSearchFilterBox

--************************************************************
function MseUpdateTermDestinationButtons()
	if termDestinationEditBox == Controls.MapSearchBox then
		--print( "termDestinationEditBox == Controls.MapSearchBox" );
		Controls.MSEBtnSelSearchBar:SetHide( false );
		Controls.MSEBtnSelFilterBar:SetHide( true );

		Controls.MseResSelTitle:LocalizeAndSetText( "LOC_MSE_EXT_WINDOW_TITLE" );
		Controls.MseCivSelTitle:LocalizeAndSetText( "LOC_MSE_EXT_WINDOW_TITLE" );
		Controls.MseUniSelTitle:LocalizeAndSetText( "LOC_MSE_EXT_WINDOW_TITLE" );
		Controls.MseImpSelTitle:LocalizeAndSetText( "LOC_MSE_EXT_WINDOW_TITLE" );
	else
		--print( "termDestinationEditBox == Controls.MapSearchFilterBox" );
		Controls.MSEBtnSelSearchBar:SetHide( true );
		Controls.MSEBtnSelFilterBar:SetHide( false );

		Controls.MseResSelTitle:LocalizeAndSetText( "LOC_MSE_EXT_WINDOW_TITLE_FILTER" );
		Controls.MseCivSelTitle:LocalizeAndSetText( "LOC_MSE_EXT_WINDOW_TITLE_FILTER" );
		Controls.MseUniSelTitle:LocalizeAndSetText( "LOC_MSE_EXT_WINDOW_TITLE_FILTER" );
		Controls.MseImpSelTitle:LocalizeAndSetText( "LOC_MSE_EXT_WINDOW_TITLE_FILTER" );
	end
end

--************************************************************
function MseSetDestinationSearch()
	termDestinationEditBox = Controls.MapSearchBox;
	MseUpdateTermDestinationButtons();
	OnEditBoxFocusGained(); -- call base function
end

--************************************************************
function MseSetDestinationFilter()
	termDestinationEditBox = Controls.MapSearchFilterBox;
	MseUpdateTermDestinationButtons();
	OnEditBoxFocusGained(); -- call base function
end
...
-- =================================================================================================================================================================================================================================
-- Zur13 Quick search terms boxes
-- =================================================================================================================================================================================================================================

include( "MseQuickResources" );

include( "MseQuickCivilizations" );

include( "MseQuickUnits" );

include( "MseQuickImprov" );

include( "MseQuickWonders" );

include( "MseQuickPanelsGeneral" ); -- should be included last (after all quick panels)

-- =================================================================================================================================================================================================================================
-- Zur13 Other
-- =================================================================================================================================================================================================================================

Z_BASE_PopulateSuggestionData = PopulateSuggestionData;
...
function PopulateSuggestionData()
	...
	-- Zur13
	AddSearchTerm("LOC_CIVILIZATION_BARBARIAN_ADJECTIVE"); 
	AddSearchTerm("LOC_MSE_EXT_SEARCH_TERMS_IMPROVEMENT_DOES_NOT_MATCH_RESOURCE"); 

	print(" MSE Mod Additional Suggestion Data added");
end

--************************************************************
local function MseResetSearchHistory()
	if m_SearchHistory ~= nil then
		print("                               MSE Mod Updating Default Search History   ");
		m_SearchHistory = { };
		table.insert(m_SearchHistory, Locale.Lookup("LOC_RESOURCE_NAME"));
		table.insert(m_SearchHistory, Locale.Lookup("LOC_TOOLTIP_STRATEGIC_RESOURCE"));
		table.insert(m_SearchHistory, Locale.Lookup("LOC_TOOLTIP_LUXURY_RESOURCE"));
		--table.insert(m_SearchHistory, Locale.Lookup("LOC_TOOLTIP_ARTIFACT_RESOURCE"));
		--table.insert(m_SearchHistory, Locale.Lookup("LOC_RESOURCE_ANTIQUITY_SITE_NAME"));
		table.insert(m_SearchHistory, Locale.Lookup("LOC_IMPROVEMENT_GOODY_HUT_NAME"));
		table.insert(m_SearchHistory, Locale.Lookup("LOC_NATURAL_WONDER_NAME"));
		--table.insert(m_SearchHistory, Locale.Lookup("LOC_UNIT_NAME"));
		table.insert(m_SearchHistory, Locale.Lookup("LOC_CIVILIZATION_BARBARIAN_ADJECTIVE"));
		table.insert(m_SearchHistory, Locale.Lookup("LOC_TOOLTIP_RIVER"));
	end

	if m_FilterHistory ~= nil then
		m_FilterHistory = { };
		table.insert(m_FilterHistory, Locale.Lookup("LOC_RESOURCE_NAME"));
		table.insert(m_FilterHistory, Locale.Lookup("LOC_TOOLTIP_STRATEGIC_RESOURCE"));
		table.insert(m_FilterHistory, Locale.Lookup("LOC_TOOLTIP_LUXURY_RESOURCE")); 
		table.insert(m_FilterHistory, Locale.Lookup("LOC_TOOLTIP_BONUS_RESOURCE")); 
		table.insert(m_FilterHistory, Locale.Lookup("LOC_TOOLTIP_ARTIFACT_RESOURCE")); 
		table.insert(m_FilterHistory, Locale.Lookup("LOC_RESOURCE_ANTIQUITY_SITE_NAME")); 
	end
end

--************************************************************
function RemoveLastWord( text )
	--print("RemoveLastWord", 1, text);
	local resText = text;
	...
	if text ~= nil then
		...
		for i, word in ipairs( words ) do
			if i < #words then
				...
				--print("    RemoveLastWord", 2, word);
			end
		end
	end
	--print("RemoveLastWord", 3, resText);
	return resText;
end

--************************************************************
local function OnMseRemoveLastWordSearchClick()
	local text = Controls.MapSearchBox:GetText();
	text = RemoveLastWord( text );
	if text ~= nil then
		Controls.MapSearchBox:SetText( text );
	end
end

--************************************************************
local function OnMseRemoveLastWordFilterClick()
	local text = Controls.MapSearchFilterBox:GetText();
	text = RemoveLastWord( text );
	if text ~= nil then
		Controls.MapSearchFilterBox:SetText( text );
	end
end

ZBASE_OnMapSearchPanelClosed = OnMapSearchPanelClosed;
ZBASE_OnSearchCommit = OnSearchCommit;

-- ===========================================================================
-- ===========================================================================
function OnMapSearchPanelClosed()
	--print(" MSE OnMapSearchPanelClosed ");
	ZBASE_OnMapSearchPanelClosed();
...
-- ===========================================================================
function OnSearchCommit()
	ZBASE_OnSearchCommit();
	MseHideAllSelectorPanels();
end

--************************************************************
function MseInitialize()	
	Controls.MSEBtnSelSearchBar:RegisterCallback( Mouse.eLClick, MseSetDestinationFilter ); -- yes this is correct one to swap destinations
	Controls.MSEBtnSelFilterBar:RegisterCallback( Mouse.eLClick, MseSetDestinationSearch ); -- yes this is correct one to swap destinations
	...
	Controls.MseRemoveLastWordSearch:RegisterCallback( Mouse.eLClick, OnMseRemoveLastWordSearchClick );
	Controls.MseRemoveLastWordFilter:RegisterCallback( Mouse.eLClick, OnMseRemoveLastWordFilterClick );
	
	InitMseSearchOpitonsPanel();

	MseInitializeQuickPanels();
end
