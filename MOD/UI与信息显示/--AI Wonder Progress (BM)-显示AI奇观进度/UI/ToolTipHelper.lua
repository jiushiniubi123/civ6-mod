...
-- Wonder Progress Helper for AI Wonder Progress
function GetWonderProgressWorldwide(buildingHash,playerId)
    local pWonderWorldProgress={};
    for i = 0, GameDefines.MAX_PLAYERS-1, 1 do
	    local pPlayer = Players[i];
        local pPlayerConfig = PlayerConfigurations[i];
		if (pPlayer:WasEverAlive() and i ~= playerId )  then
					...
					if (pCity ~= nil) then
						...
						if (cityBuildHash ~= nil) then
							if (cityBuildHash == buildingHash) then
                                local pCivilizationName = "";
                                local pCityName = "";
                                if (pPlayer:GetDiplomacy():HasMet() == true) then
                                    pCivilizationName = "LOC_"..pPlayerConfig:GetCivilizationTypeName().."_NAME";
                                    pCityName = pCity:GetName();
                                else
                                    pCivilizationName = "LOC_HUD_WONDERPROGRESS_TOOLTIP_UNKNOWNCIV";
                                    pCityName = "LOC_HUD_WONDERPROGRESS_TOOLTIP_UNKNOWNCITY";
                                end
                                local bCivilizationName = "[ICON_Production]"..Locale.Lookup(pCivilizationName).." ";
                                local bCityName = " "..Locale.Lookup(pCityName).." ";
                                local bTurnsLeft = "[ICON_turn]"..pCity:GetBuildQueue():GetTurnsLeft().." ";
                                local pWonderProgress = {bCivilizationName, bCityName, bTurnsLeft};
                                table.insert(pWonderWorldProgress, pWonderProgress);
                            end
						end
					end
				end	
...
ToolTipHelper.GetBuildingToolTip = function(buildingHash, playerId, city)
	...
	if (building.IsWonder == true) then
        local pWonderWorldProgress = GetWonderProgressWorldwide(buildingHash, playerId);
        if (#pWonderWorldProgress >= 1) then
            table.insert(toolTipLines, " ");
            for i,v in pairs(pWonderWorldProgress) do
                local pTooltips1 = Locale.Lookup(v[1])..Locale.Lookup("LOC_HUD_WONDERPROGRESS_TOOLTIP_1")..Locale.Lookup(v[2])..Locale.Lookup("LOC_HUD_WONDERPROGRESS_TOOLTIP_2");
                local pTooltips2 = "[ICON_Bullet]"..v[3]..Locale.Lookup("LOC_HUD_WONDERPROGRESS_TOOLTIP_3");
                table.insert(toolTipLines, pTooltips1);
                table.insert(toolTipLines, pTooltips2);
            end
        end
    end
