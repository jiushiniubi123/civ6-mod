...
function UpdateGreatWorks()
	...
	-- GWV ----------------------------------------------------------------
	-- To sort the great works, all building data is first collected,
			...
			for buildingInfo in GameInfo.Buildings() do
				...
				if(pCityBldgs:HasBuilding(buildingIndex)) then
					...
					if (numSlots ~= nil and numSlots > 0) then
						...
						thisBuilding.name = Locale.Lookup(buildingInfo.Name);
	...
	-- Resize window to fit more columns, if possible.
        GwvResize(table.count(allBuildings))
	-- GWV END ------------------------------------------------------------
		...
		if pCityBldgs:IsBuildingThemedCorrectly(GameInfo.Buildings[pBuildingInfo.BuildingType].Index) then
			...
			elseif pBuildingInfo.BuildingType == "BUILDING_MUSEUM_ARTIFACT" then
				...
				if firstGreatWork == greatWorkIndex then
					...
					-- GWV: A Heroic Relic might end up first in a museum, but has no EraType.
					local apa = GameInfo.GreatWorks[firstGreatWorkObjectTypeID].EraType;
					if apa == nil then
						print("Workaround, skipping theming info");
						return strBasicTooltip;
					end
...
function GetGreatWorkBuildings() -- GWV
	return m_GreatWorkBuildings;
end
