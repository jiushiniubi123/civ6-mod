...
("Tech", 2, NULL, 1.3, 1.177, 1.082),
("Tech", 3, NULL, 1.7, 1.39, 1.173),
("Tech", 4, NULL, 2.2, 1.63, 1.267),
("Tech", 5, NULL, 2.6, 1.808, 1.332),
("Tech", 6, NULL, 3.2, 2.057, 1.418),
("Tech", 7, NULL, 4, 2.362, 1.516),
("Tech", 8, NULL, 4.6, 2.576, 1.581),
("Tech", 9, NULL, 5.5, 2.878, 1.668);
...
ELSE Cost * (-4.0 + ((SELECT ChronologyIndex FROM Eras WHERE Eras.EraType = Technologies.EraType LIMIT 1) * 1.0))
END
...
-- Fallback Scaling Formula for Unknown Eras
ELSE Cost * (-4.0 + ((SELECT ChronologyIndex FROM Eras WHERE Eras.EraType = (SELECT Technologies.EraType FROM Technologies WHERE Technologies.TechnologyType = TechnologyRandomCosts.TechnologyType LIMIT 1) LIMIT 1) * 1.0))
END
