...
("Tech", 2, NULL, 1.2, 1.12, 1.056),
("Tech", 3, NULL, 1.4, 1.232, 1.106),
("Tech", 4, NULL, 1.6, 1.338, 1.151),
("Tech", 5, NULL, 1.8, 1.44, 1.193),
("Tech", 6, NULL, 2, 1.537, 1.231),
("Tech", 7, NULL, 2.3, 1.676, 1.284),
("Tech", 8, NULL, 2.6, 1.808, 1.332),
("Tech", 9, NULL, 2.9, 1.935, 1.376);
...
ELSE Cost * (0.0 + ((SELECT ChronologyIndex FROM Eras WHERE Eras.EraType = Technologies.EraType LIMIT 1) * 0.32))
END
...
-- Fallback Scaling Formula for Unknown Eras
ELSE Cost * (0.0 + ((SELECT ChronologyIndex FROM Eras WHERE Eras.EraType = (SELECT Technologies.EraType FROM Technologies WHERE Technologies.TechnologyType = TechnologyRandomCosts.TechnologyType LIMIT 1) LIMIT 1) * 0.32))
END
