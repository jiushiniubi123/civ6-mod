...
("Civic", 3, NULL, 1.15, 1.091, 1.043),
("Civic", 4, NULL, 1.2, 1.12, 1.056),
("Civic", 5, NULL, 1.25, 1.148, 1.069),
("Civic", 6, NULL, 1.3, 1.177, 1.082),
("Civic", 7, NULL, 1.38, 1.221, 1.101),
("Civic", 8, NULL, 1.45, 1.259, 1.118),
("Civic", 9, NULL, 1.5, 1.286, 1.129);
...
ELSE Cost * (0.6 + ((SELECT ChronologyIndex FROM Eras WHERE Eras.EraType = Civics.EraType LIMIT 1) * 0.1))
END
...
-- Fallback Scaling Formula for Unknown Eras
ELSE Cost * (0.6 + ((SELECT ChronologyIndex FROM Eras WHERE Eras.EraType = (SELECT Civics.EraType FROM Civics WHERE Civics.CivicType = CivicRandomCosts.CivicType LIMIT 1) LIMIT 1) * 0.1))
END
