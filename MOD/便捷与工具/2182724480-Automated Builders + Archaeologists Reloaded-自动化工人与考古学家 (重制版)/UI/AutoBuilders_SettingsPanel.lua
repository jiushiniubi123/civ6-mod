...
local m_kPrioritiesIM_ImprovementUniquesScoring	= InstanceManager:new( "PriorityInstance",					"Top",	Controls.PrioritiesStack_ImprovementUniquesScoring );
...
m_kPrioritiesIM_ImprovementUniquesScoring:ResetInstances();
...
for row in GameInfo.CNO_AutoBuilders_PrioritySettings() do
	...
	elseif row.mTable == "Improvement_Scoring" then
		if GameInfo.Improvements[row.Setting] ~= nil then
			local traitType = GameInfo.Improvements[row.Setting].TraitType
			local traitLeader = GetLeaderFromTrait(traitType)
			local traitCiv = GetCivFromTrait(traitType)
			
			-- traitType == "TRAIT_CIVILIZATION_NO_PLAYER" refers to Governor improvements, leave them with the basic improvements.
			local isUnique = traitLeader or traitCiv
						
			local kInstance :table = nil
			if isUnique then
				-- If is a civ unique improvement, check we are the right civilization. 
				if string.find(traitType, "MINOR_CIV") or traitCiv == nil or (PlayerConfigurations[Game.GetLocalPlayer()]:GetCivilizationTypeName() == traitCiv[1]) then 
					kInstance = m_kPrioritiesIM_ImprovementUniquesScoring:GetInstance();
				end
			else
				kInstance = m_kPrioritiesIM_ImprovementScoring:GetInstance();
			end
			
			if kInstance ~= nil then
