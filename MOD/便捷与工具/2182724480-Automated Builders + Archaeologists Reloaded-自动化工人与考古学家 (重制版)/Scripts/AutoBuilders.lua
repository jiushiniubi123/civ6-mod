...
CHANGELOG
Update 09 May, 2021
Version 0.99.10 (Adriaman)
Improved compatibility with mods changing unit availability.

Update 26 April, 2021
Version 0.99.9 (Adriaman)
Added custom conditions for Moai improvements.
Fixed another bug related to lumbermills on woods/rainforests.

Update 19 March, 2021
Version 0.99.8 (Adriaman)
Fixed bug added in the previous update that broke support for non-Gathering Storm games.

Update 14 March, 2021
Version 0.99.7 (Adriaman)
Added check for flooded tiles.

Update 24 February, 2021
Version 0.99.6 (Adriaman)
Fixed another bug that prevented builders from replacing improvements.

Update 01 February, 2021
Version 0.99.5 (Adriaman)
Fixed bug that prevented builders from replacing improvements.

Update 31 January, 2021
Version 0.99.4 (Adriaman)
Prevented builders from building industries (they have to be manually controlled).

Update 27 January, 2021
Version 0.99.3 (Adriaman)
Fixed bug that made the Mercantilism civic condition to also apply to woods instead of just rainforests.
Fixed bug that made the builder try building again the same improvement in some cases (e.g. undiscovered uranium on a wood plot improved with a lumber mill).

Update 23 January, 2021
Version 0.99.2 (Adriaman)
Added more improvement settings.

Update 18 December, 2020
Version 0.99.1 (Adriaman)
Fixed terrain adjacency bonuses.

Version 0.99 (Adriaman)
Added workaround to improvement / unique improvement plot bug. The game engine gives incomplete plot improvement info for some improvements (e.g. sphinx, seastead, offshore wind farm) which causes autobuilder to fail when adjacency checks are required.
Added workaround for Gathering Storm improvements, which have a bugged id in the game engine.
Fixed lumbermill on rainforest bug due to Mercantilism civic.
Fixed minor bugs.
Updated for Gathering Storm and Frontier Pass.

Version 0.98.2 (eudaimonia)
Script Version: 0.97
...
local b_isGatheringStormActive = Modding.IsModActive("4873eb62-8ccc-4574-b784-dda455e74e68"); -- Gathering Storm
...
-- Adriaman
-- Some Gathering Storm improvements do not have the "Buildable" flag nor the "EnforceTerrain" flag. So we add them as a separate class.
BUILDABLE_EXCEPTIONS = {"IMPROVEMENT_GEOTHERMAL_PLANT", "IMPROVEMENT_SOLAR_FARM", "IMPROVEMENT_WIND_FARM", "IMPROVEMENT_OFFSHORE_WIND_FARM", "IMPROVEMENT_SEASTEAD"}
...
local function GetGameImprovements()
	...
	-- Manually add Gathering Storm improvements
	...
	for row in GameInfo.Improvements() do 
		...
		--[[
		print("----")
		print("Name: " .. tostring(row.Name))
		print("Hash: " .. tostring(row.Hash))
		print("Buildable: " .. tostring(row.Buildable))
		print("EnforceTerrain: " .. tostring(row.EnforceTerrain))
		print("ImprovementType: " .. tostring(row.ImprovementType))
		print("PrereqTech: " .. tostring(row.PrereqTech))
		print("PrereqCivic: " .. tostring(row.PrereqCivic))
		print("BuildOnFrontier: " .. tostring(row.BuildOnFrontier))
		print("RequiresRiver: " .. tostring(row.RequiresRiver))
		print("SameAdjacentValid: " .. tostring(row.SameAdjacentValid))
		print("Coast: " .. tostring(row.Coast))
		print("MinimumAppeal: " .. tostring(row.MinimumAppeal))
		print("Domain: " .. tostring(row.Domain))
		print("OnePerCity: " .. tostring(row.OnePerCity))
		print("RequiresAdjacentBonusOrLuxury: " .. tostring(row.RequiresAdjacentBonusOrLuxury))
		print("ValidAdjacentTerrainAmount: " .. tostring(row.ValidAdjacentTerrainAmount))
		print("AdjacentSeaResource: " .. tostring(row.AdjacentSeaResource))
		print("TraitType: " .. tostring(row.TraitType))
		-- ]]
			
		-- Only add buildable improvements
		-- Adriaman: add exceptions
		isException = false
		
		if (row.Buildable or row.EnforceTerrain) or TableContains(BUILDABLE_EXCEPTIONS, row.ImprovementType) then 
			for buildUnitRow in GameInfo.Improvement_ValidBuildUnits() do -- Check Valid Build Units.
				...
				if row.ImprovementType == buildUnitRow.ImprovementType and buildUnitRow.UnitType == "UNIT_BUILDER" then
					...
					-- Adriaman: fix adjacency terrain count (make sure these variables exist).
					if row.ValidAdjacentTerrainAmount == true then
						o.ADJACENT_VALID_TERRAIN = true;
						o.ADJACENT_VALID_TERRAIN_COUNT=1; 
					end
					if type(row.ValidAdjacentTerrainAmount) == "number" and row.ValidAdjacentTerrainAmount >= 1 then 
						o.ADJACENT_VALID_TERRAIN = true;
						o.ADJACENT_VALID_TERRAIN_COUNT=row.ValidAdjacentTerrainAmount;
					end
...
function BuilderClass:removeImprovement()
	...
	UnitManager.RequestOperation( UnitManager.GetUnit( self:getOwner(), self.ID ), UNIT_OPS["UNITOPERATION_REMOVE_IMPROVEMENT"].HASH );
...
local function RemoveLastBan()
	if next(m_BannedPlotActions.X) ~= nil then
		Debug( "Removing BANNED action: " .. m_BannedPlotActions.Actions[#m_BannedPlotActions.Actions] .. " for type: " .. m_BannedPlotActions.ActionTypes[#m_BannedPlotActions.ActionTypes] .. 
					" on plot: x=" .. m_BannedPlotActions.X[#m_BannedPlotActions.X] .. "; y=" .. m_BannedPlotActions.Y[#m_BannedPlotActions.Y] .. " for automated Builders for this session." );
		
		table.remove(m_BannedPlotActions.X);
		table.remove(m_BannedPlotActions.Y);
		table.remove(m_BannedPlotActions.Actions);
		table.remove(m_BannedPlotActions.ActionTypes);
	end
end
...
function BuilderClass:performAction()
	...
	if self.queuedAction then -- Attempt to perform the Builder's queued action.
		...
		if unit then
			...
			-- Do a check if previous (different) action was successful. Can't check right after action taken as a UI request is sent, code isn't run right away.
			...
			--[[ 
				Adriaman:
				Quick and dirty workaround for Gathering Storm issues.
				For some reason the game engine always returns -1 (instead of the expected id) for the improvements in BUILDABLE_EXCEPTIONS when calling aPlot:getImprovementType(). This messes up adjacency tests. The seastead is the only one to have adjacency conditions, the seastead. 
				
				So we automatically check if a builder charge was used here. If not, it means that it failed so we ban the seastead improvement from the plot. This somewhat breaks encapsulation, but it is really a difficult situation to solve without adding complex conditions. The builder should build another building, likely the offshore wind farm. 
			-- ]]
			if (self.queuedAction == "BUILD_IMPROVEMENT" or self.queuedAction == "BUILD_UNIQUE_IMPROVEMENT") then
				-- If the build charge did not change, it indicates failure.
				-- Restrict fix to adjacency issues
				if self.improvement.ADJACENT == false and unit:GetBuildCharges() ~= buildsLeft then
					Error( "Builder (" .. m_Builder.ID .. ") could not build " .. self.improvement.NAME .. ", likely due to adjacency issues (it is a game engine bug). It will attempt to build another improvement.");
					
					BanPlotAction( self.targetPlot, self.queuedAction, self.improvement.NAME );
					self:getAction();
					self:realizeAction();
					-- Check if builder did an action on this plot. If yes, remove the ban (in case the improvements are removed later).
					-- If it fails again, normal ban procedure should take care of it.
					if (self.targetPlot:GetX() == self:x()) and (self.targetPlot:GetY() == self:y()) then
						RemoveLastBan()
					end
				end
			end
...
--[[
	print("Valid Terrains")
	
	for row in GameInfo.Improvement_ValidTerrains() do
		print("-- " .. row.ImprovementType)
	end
	
	print("Player improvements")
	for _, anImprovement in pairs( m_PlayerImprovements[m_Player:GetID()] ) do
		print("-- " .. anImprovement.NAME)
	end
]]	
			...
			if anImprovement.NAME == row.ImprovementType then -- Check for the improvement on the list.
				...
				-- For now, EnforceTerrain is used for the Oil improvements. Even though they are on ValidTerrains, don't include because they improve Oil.
					if terrain == GameInfo.Terrains[row.TerrainType].Index then -- Check if the terrain tag matches input terrain.
						Debug_AllActions(anImprovement.NAME); 
...
local function IsPlotAdjacentToImprovement( aPlot, improvement )
	...
	local x_s = {0, -1, 1, -1, 0, -1}; -- x changes for adjacent hexes.
	local y_s = {1, 1, 0, 0, -1, -1};
	-- print("AA" .. tostring(GameInfo.Improvements[improvement.NAME].Index) .. " (" .. improvement.NAME .. ")")
	for key, x_change in pairs( x_s ) do
		local x = x_start + x_s[key];
		local y = y_start + y_s[key];
		x = rescale( x, map_width, false ); -- Rescale x around based on map width.
		...
		if plot then
			local plot_improvement = plot:GetImprovementType();
			if improvement then
				-- print("(" .. tostring(x) .. "," .. tostring(y) .. ") " .. tostring(improvement) .. " -- " .. tostring(plot_improvement))
				if plot_improvement == improvement then 
					return true;
				end
			end
...
--------------------------------------------------------------------------------------------------------
-- Returns true if the aPlot is adjacent to a sea resource
local function IsPlotAdjacentToFeature( aPlot, aFeature, minimum )
	local x_start = aPlot:GetX();
	local y_start = aPlot:GetY();
	local x_s = {0, -1, 1, -1, 0, -1}; -- x changes for adjacent hexes.
	local y_s = {1, 1, 0, 0, -1, -1};
	...
	for key, x_change in pairs( x_s ) do
		...
		if plot then
			...
			if feature > -1 then
				Debug_AllActions( "Plot (" .. x .. ',' .. y .. ") has feature type: " .. Locale.Lookup( GameInfo.Features[feature].Name ) .. " (" .. GameInfo.Features[feature].PrimaryKey .. ')');
				if GameInfo.Features[feature].PrimaryKey == aFeature then 
					if minimum == nil then return true; end
	...
	if minimum == nil then return false; end
	if feature_count >= minimum then return true; else return false; end
end
...
local function ProcessImprovements_SpecialRules( Improvements, aPlot )
	...
	for key, anImprovement in ipairs( Improvements ) do -- Go through the given table of Builder Actions
		...
		-- Adriaman
		-- If the terrain is rainforest and the improvement the lumber mill, check if the mercantilism is discovered. This civic requirement is only for rainforests, so it does not appear when getting all available improvements.
		if keepImprovement and anImprovement.NAME == "IMPROVEMENT_LUMBER_MILL" then
			feature = aPlot:GetFeatureType();
			if feature ~= -1 then
				if GameInfo.Features[feature].PrimaryKey == "FEATURE_JUNGLE" and not m_PlayerCulture:HasCivic( GameInfo.Civics["CIVIC_MERCANTILISM"].Index ) then 
					keepImprovement = false; 
					Debug_AllActions("Improvement: " .. anImprovement.NAME .. " is a lumbermill on rainforest but Mercantilism has not been discovered yet at: x=" .. x_start .. " ; y=" .. y_start .. ". Removing improvement: " .. GameInfo.Improvements[anImprovement.NAME].Index .. " (" .. anImprovement.NAME .. ") from the table.");
				end
			end
		end
		
		-- Adriaman: prevent builders from building industries, which are special improvements on luxuries once two copies of the luxury is improved. They may also be built on already improved luxuries if there are only two copies of the luxury in the empire.
		if keepImprovement and anImprovement.NAME == "IMPROVEMENT_INDUSTRY" then
			keepImprovement = false
			Debug_AllActions("Improvement at: x=" .. x_start .. " ; y=" .. y_start .. " is an industry. This action must be done manually. Removing improvement: " .. GameInfo.Improvements[anImprovement.NAME].Index .. " (" .. anImprovement.NAME .. ") from the table.");
		end
		
		-- Adriaman: Moais cannot be built near Woods or Rainforest
		if keepImprovement and anImprovement.NAME == "IMPROVEMENT_MOAI" then
			if IsPlotAdjacentToFeature(aPlot, "FEATURE_JUNGLE") then
				keepImprovement = false
				Debug_AllActions("Improvement at: x=" .. x_start .. " ; y=" .. y_start .. " is a Moai. It cannot be built near woods. Removing improvement: " .. GameInfo.Improvements[anImprovement.NAME].Index .. " (" .. anImprovement.NAME .. ") from the table.");
			elseif IsPlotAdjacentToFeature(aPlot, "FEATURE_FOREST") then
				keepImprovement = false
				Debug_AllActions("Improvement at: x=" .. x_start .. " ; y=" .. y_start .. " is a Moai. It cannot be built near rainforests. Removing improvement: " .. GameInfo.Improvements[anImprovement.NAME].Index .. " (" .. anImprovement.NAME .. ") from the table.");
			end
		end
		...
		if keepImprovement and anImprovement.ADJACENT_VALID_TERRAIN then
			...
			if count < anImprovement.ADJACENT_VALID_TERRAIN_COUNT then
				Debug_AllActions("Found that plot: x=" .. x_start .. " ; y=" .. y_start .. 
				" is not adjacent to plots of valid terrain types (" .. anImprovement.ADJACENT_VALID_TERRAIN_COUNT .. " tile(s) needed). Removing improvement: " .. 
				GameInfo.Improvements[anImprovement.NAME].Index .. " (" .. anImprovement.NAME .. ") from the table.");
				keepImprovement = false;
			end
		end
			...
			if district == -1 then -- Plot has no district
				if aPlot:IsMountain() == false then -- Plot has no mountain
					...
					if improvement == -1 then -- Plot has no improvement
						...
						Debug_AllActions( "Plot already has an improvement (" .. GameInfo.Improvements[improvement].ImprovementType .. ")." );
							...
							if feature == -1 then -- Plot has an improvement, a resource, and no feature
								...
								if validActions then
									for key, anAction in pairs( validActions ) do
										...
										-- Adriaman: INVISIBLE_FEATURE and INVISIBLE_NO_FEATURE are magic values here, not from the game engine.
										if resourceData == "INVISIBLE_FEATURE" then -- Resource is invisible for player, plot has feature.
											-- Only add an action if the current plot improvement doesn't match the improvement valid for this resource.
											if featureData then
												action = featureData.TYPE;
										...
										-- Adriaman: for some reason the table insert was missing.
										table.insert( returnedActionTable, actionO );
									end
								end
							end
					...
					-- Adriaman: we need to filter actions as it sometimes tries to build again the same improvement.
					-- For instance, if there was invisible uranium in woods with an already built lumber mill, the builder would still try to build another lumber mill. So we check if the improvement to build is different.
					filteredReturnedActionTable = {}
					for key, action in ipairs( returnedActionTable ) do
						-- Check if we are tring to build an improvement
						if improvement ~= -1 and action.action_type == "BUILD_IMPROVEMENT" and action.improvement and  action.improvement.NAME and type(improvement) == "number" then							
							if action.improvement.NAME ~= GameInfo.Improvements[improvement].ImprovementType then
								table.insert( filteredReturnedActionTable, action );
							else
								Debug_AllActions("AutoBuilder Debug: tried to build again the improvement " .. action.improvement.NAME .. " on plot (x=" .. aPlot:GetX() .. " ; y=" .. aPlot:GetY() .. "). Removing from table." );
							end
						else
							table.insert( filteredReturnedActionTable, action );
						end
					...
					returnedActionTable = filteredReturnedActionTable
					...
					if debug_AllActions then -- Print any valid actions found for a Builder.
						...
						for key, action in ipairs( returnedActionTable ) do 
							if not action.improvement or not action.improvement.NAME or type(improvement) == "number" then
...
local function CheckUnitType(unitType, key)
	if unitType ~= nil and GameInfo.Units[key] ~= nil then
		return unitType == GameInfo.Units[key].Index
	end
...
local function IsCivilian( Unit )
	...
	if CheckUnitType(unitType, "UNIT_SETTLER") then isCivilian = true;
	elseif CheckUnitType(unitType, "UNIT_BUILDER") then isCivilian = true;
	elseif CheckUnitType(unitType, "UNIT_BUILDER") then isCivilian = true;
	elseif CheckUnitType(unitType, "UNIT_NATURALIST") then isCivilian = true;
	elseif CheckUnitType(unitType, "UNIT_GREAT_GENERAL") then isCivilian = true;
	--elseif CheckUnitType(unitType, "UNIT_GREAT_ADMIRAL") then isCivilian = true;
	elseif CheckUnitType(unitType, "UNIT_GREAT_ENGINEER") then isCivilian = true;
	elseif CheckUnitType(unitType, "UNIT_GREAT_MERCHANT") then isCivilian = true;
	elseif CheckUnitType(unitType, "UNIT_GREAT_PROPHET") then isCivilian = true;
	elseif CheckUnitType(unitType, "UNIT_GREAT_SCIENTIST") then isCivilian = true;
	elseif CheckUnitType(unitType, "UNIT_GREAT_WRITER") then isCivilian = true;
	elseif CheckUnitType(unitType, "UNIT_GREAT_ARTIST") then isCivilian = true;
	elseif CheckUnitType(unitType, "UNIT_GREAT_MUSICIAN") then isCivilian = true;
	elseif 
...
local function IsReligious( Unit )
	...
	local unitType = Unit:GetUnitType();
	if CheckUnitType(unitType, "UNIT_MISSIONARY") then isReligious = true;
	elseif CheckUnitType(unitType, "UNIT_APOSTLE") then isReligious = true;
	elseif CheckUnitType(unitType, "UNIT_INQUISITOR") then isReligious = true;
	-- Only check for a Guru if the fall 2017 patch is installed.
	elseif b_Fall2017Patch_Installed and CheckUnitType(unitType, "UNIT_GURU") then isReligious = true;
...
local function IsTrader( Unit )
	...
	local unitType = Unit:GetUnitType();
	if CheckUnitType(unitType, "UNIT_TRADER") then isTrader = true; 
...
local function IsSpy( Unit )
	local isSpy = false;
	local unitType = Unit:GetUnitType();
	if CheckUnitType(unitType, "UNIT_SPY") then isSpy = true;
	elseif GameInfo.Units[ unitType ].FormationClass == "FORMATION_CLASS_CIVILIAN" and 
...
local function CanMoveToPlot( unit, aPlot )
	...
	if b_isGatheringStormActive then
		-- This function is available only in Gathering Storm!
		if TerrainManager.IsFlooded(aPlot) then
			Debug_AllActions( "Plot x=" .. aPlot:GetX() .. " ; y=" .. aPlot:GetY() .. " is flooded! Builder cannot build here." );
			return false; 
		end 
		
		-- Adriaman: submerged tiles can be improved as they are actually coastal tiles.
		--[[
		if TerrainManager.IsSubmerged(aPlot) then
			Debug_AllActions( "Plot x=" .. aPlot:GetX() .. " ; y=" .. aPlot:GetY() .. " is submerged! Builder cannot build here." );
			return false; 
		end 
		--]]
	end
...
local function BM_GetAvailableBuilders( builders )
	...
	for _j, builder in ipairs( builders ) do
		if builder.assigned == false then -- eligible for targeting.
			...
			if unit and CheckUnitType(unit:GetUnitType(), "UNIT_BUILDER") then
...
local function OnUnitMoveComplete( iPlayer, iUnit )
	local unit = nil;
	if UnitManager then unit = UnitManager.GetUnit( iPlayer, iUnit ); end
	if unit and CheckUnitType(unit:GetUnitType(), "UNIT_BUILDER") 
