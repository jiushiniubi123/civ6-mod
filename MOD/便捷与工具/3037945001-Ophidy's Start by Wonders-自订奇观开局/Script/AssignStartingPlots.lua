...
function AssignStartingPlots:__InitStartingData()
	...
	for i = self.iNumMajorCivs - 1, 0, - 1 do
		...
		if(startPlot ~= nil) then
			...
			print('major start plot insert '..startPlot:GetX(),startPlot:GetY())
...
--			print ("Minor SouthEdge: " .. tostring(info.SouthEdge));
	...
	if startWondersConfig and #startWondersConfig > 0 then
		...
		function Wonder_Start( tHumanPlayer )
			...
			-- Randomly Sorted Wonders Generated
			local tNW			=	{};
			...
			-- Stop If No Enough Wonders
			if #tNW >= #tHumanPlayer then 
														
				-- Get Tiles 4 Tiles Away Wonders
				local reserveplots =	{};
				for iPlotIndex = 0, Map.GetPlotCount()-1, 1 do	
					local pPlot = Map.GetPlotByIndex(iPlotIndex)
					local b = true
				...
				-- Randomly Distribute Wonders To All Human Players
				local PlayerWonders = {}
				...
				-- Get All Players
				local tAllAlivePlayerIDs = {}
				for i, id in ipairs(PlayerManager:GetAliveMajorIDs()) do
					table.insert(tAllAlivePlayerIDs, id)
				end
				for i, id in ipairs(PlayerManager:GetAliveMinorIDs()) do
					table.insert(tAllAlivePlayerIDs, id)
				end
				...
				for index, row in ipairs(PlayerWonders) do
					...
					-- Coastalwater Plot Within 2 Tiles is better than Freshwater Plot Within 3 Tiles
					...
					-- No Available Plot (Commonly Is Bermuda)
					if not bestlocation then 
					...
					--[[-- ???????
					WS_Row = {}
					WS_Row.PlayerID = playerID
					WS_Row.StartPlot = bestlocation
					table.insert(tHumanStartPlot_WS, WS_Row)]]--
					
					-- Eject AI Player
					for	j, pPlot in ipairs(GetValidPlotsInRadiusR(bestlocation, 7)) do
						for i, id in ipairs(tAllAlivePlayerIDs) do
							local mPlayer	= Players[id];
							local mPlot		= mPlayer:GetStartingPlot();
							if id ~= playerID and not mPlayer:IsHuman() then
								if mPlot == pPlot then
									local ejectplot = mPlot
									if mPlayer:IsMajor() then
										--plots = StartPositioner.GetMajorCivStartPlots(id);
										--local startPlot = AssignStartingPlots:__SetStartMajor(plots, id);
										ejectplot = InPlotsFarthestToPlot(bestlocation, GetValidPlotsInRadiusR(mPlot, 5), 1, 12).plot
										if ejectplot == bestlocation then
											ejectplot = mPlot
										end
										mPlayer:SetStartingPlot(ejectplot)
									else 
										--plots = StartPositioner.GetMinorCivStartPlots(id);
										--local startPlot = AssignStartingPlots:__SetStartMinor(plots);
										ejectplot = InPlotsFarthestToPlot(bestlocation, GetValidPlotsInRadiusR(mPlot, 3), 1, 10).plot
										if ejectplot == bestlocation then
											ejectplot = mPlot
										end
										mPlayer:SetStartingPlot(ejectplot)
									end
									print('AI player '..id.."'s units is successfully ejected from "..mPlot:GetX(),mPlot:GetY()..' to '..ejectplot:GetX(),ejectplot:GetY())
								end
							end
						end
					end
					...
					pPlayer:SetStartingPlot(bestlocation)
				end
			end
		end
		Wonder_Start_Initial()
	end
end
