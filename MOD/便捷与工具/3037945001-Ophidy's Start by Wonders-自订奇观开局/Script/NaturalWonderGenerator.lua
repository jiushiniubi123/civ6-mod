...
function NaturalWonderGenerator:__InitNWData()
	...
	local startWondersConfig = GameConfiguration.GetValue("START_NATURAL_WONDERS");
	...
	if(excludeWondersConfig and #excludeWondersConfig > 0) then
		...
		for i,v in ipairs(excludeWondersConfig) do
			...
			if(startWondersConfig and #startWondersConfig > 0) then
				for j, w in ipairs(startWondersConfig) do
					if v == w then
						b = false
					end
				end
	...
	self.iNumWondersInDB = iCount;					-- 选定奇观总量
...
function NaturalWonderGenerator:__FindValidLocs()
	...
	for i = 0, iPlotCount - 1 do
		...
		for iI = 0, self.iNumWondersInDB - 1 do
			...
			if (customPlacement == nil) then
				...
				if (CustomCanHaveFeature(pPlot, self.eFeatureType[iI])) then			-- 特殊放置位置的奇观
					row = {};
					row.MapIndex = i;
					row.Score = iBaseScore;
					table.insert (self.aaPossibleLocs[iI], row);
	...
	for iI = 0, self.iNumWondersInDB - 1 do
		local iNumEntries = #self.aaPossibleLocs[iI];									-- 能放置该奇观的单元格量
		...
		if (iNumEntries > 0) then
			...
			selectionRow.RandomScore = TerrainBuilder.GetRandomNumber (100, "Natural Wonder Selection Roll"); -- 随机量？

			if(startWondersConfig and #startWondersConfig > 0) then
				for j, w in ipairs(startWondersConfig) do
					if self.eFeatureType[iI] == GameInfo.Features[w].Index then
						print('Boost up '..self.eFeatureType[iI])
						selectionRow.RandomScore = 100
					end
				end
			end
	...
	table.sort(self.aSelectedWonders, function(a, b) return a.RandomScore > b.RandomScore; end);	-- 将可生成的奇观随机排序
...
function NaturalWonderGenerator:__ScorePlots(NWIndex)
    ...
    for i, row in ipairs(self.aaPossibleLocs[NWIndex]) do
		...
		if (iClosestConflictingDist <= 10) then
			iDistanceScore = iClosestConflictingDist * 200;
		else
			iDistanceScore = 2000 + (iClosestConflictingDist - 10);
		end
